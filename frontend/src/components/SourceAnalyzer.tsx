...
export function SourceAnalyzer({ onSuccessCallback }: SourceAnalyzerProps = {}) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 backdrop-blur-md bg-white/5 p-6 space-y-6">
      <div className="space-y-3">
        <h3 className="text-xs font-semibold text-white/70 uppercase tracking-widest">Source</h3>
        <div
          role="tablist"
      ...
      {sourceType === 'local' ? (
        <div className="space-y-3">
          <label
            ...
            className="flex items-center gap-2 text-xs font-semibold text-white/60 uppercase tracking-wide"
          >
            <span>Project path</span>
          </label>
          <div className="relative">
            <Terminal className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/40" aria-hidden />
            <input
              ...
              className="w-full pl-10 pr-4 py-3 rounded-xl bg-black/40 text-white/90 placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all font-mono text-sm disabled:opacity-60"
              disabled={isPending}
            />
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <Link className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/40" aria-hidden />
            <input
          ...
          <label
            ...
            className="block text-xs font-semibold text-white/60 uppercase tracking-wide"
          >
            Branch, tag, or commit (optional)
          </label>
          <div className="relative">
            <GitBranch className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/40" aria-hidden />
            <input
              id="source-branch"
              type="text"
              value={branch}
        ...
        <div className="space-y-3">
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-xl bg-black/40 text-white/90 focus:outline-none focus:ring-2 focus:ring-purple-500/50 font-mono text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-white/10 file:px-3 file:py-2 file:text-xs file:font-semibold file:uppercase file:tracking-wide file:text-white/80 hover:file:bg-white/20 disabled:opacity-60"
            disabled={isPending}
          />
          {zipFile && (
      ...
      <div id="source-hint" className="text-xs text-white/40 space-y-2 leading-relaxed pt-2 font-mono">
        {sourceType === 'local' ? (
          <>
            <p>Absolute path to the project directory</p>
            <p className="text-white/60">Example: /Users/you/projects/my-app</p>
            <p className="flex items-center gap-2 mt-3">
              <kbd className="px-2 py-1 bg-white/10 border border-white/10 rounded text-[10px]">
                ⌘K
              </kbd>
          ...
          <>
            ...
            <p className="text-white/60">Example: https://github.com/owner/repo</p>
          </>
        ) : (
          <>
            <p>Upload a .zip of your project directory (e.g. from Finder or Explorer).</p>
            <p className="text-white/60">Max 100 MB compressed, 500 MB uncompressed.</p>
          </>
        )}
      </div>
    </div>
