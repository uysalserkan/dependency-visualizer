...
export function ExportButton() {
  ...
  const handleExportPng = async () => {
    ...
    try {
      const result = cyInstance.png({ output: 'blob', scale: 2 })
      const blob = result instanceof Promise ? await result : result
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `graph_${analysis.id}.png`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
  ...
  return (
    <div className="relative group">
      <button
        type="button"
        aria-haspopup="true"
        className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium text-sm text-gray-700 dark:text-white border border-gray-200 dark:border-white/10 bg-transparent hover:bg-gray-100 dark:hover:bg-white/5 transition-all duration-200"
      >
      ...
      <div className="absolute right-0 mt-2 w-52 rounded-xl border border-gray-200 dark:border-white/5 bg-white dark:bg-slate-900/95 backdrop-blur-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 shadow-2xl py-2" role="menu">
        ...
        <button
          ...
          disabled={!cyInstance}
          title={!cyInstance ? 'Graph not ready' : 'Export current view as PNG'}
